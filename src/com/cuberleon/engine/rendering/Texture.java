...
import com.cuberleon.engine.rendering.resources.TextureBuffer;
...
public class Texture {
    ...
    public Texture(String filePath) {
        ...
        (m_buffer = m_loadedTextures.get(filePath).get()) != null) {
            m_buffer.addReference();
        } else {
            m_buffer = loadTexture(filePath);
            m_loadedTextures.put(filePath, new WeakReference<>(m_buffer));
        }
    }
    ...
    protected void finalize() throws Throwable {
        try {
            if (m_buffer != null && m_buffer.deleteReference()) {
                if (m_filePath != null) {
